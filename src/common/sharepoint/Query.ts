export class Query {
    ...
    public static readonly none: Query = null;

    public static id(id: number) {
        return new Query(`<Where><Eq><FieldRef Name='ID' /><Value Type='Integer'>${id}</Value></Eq></Where>`);
    }

    public static lookup(column: string, id: number) {
        return new Query(`<Where><Eq><FieldRef Name='${column}' LookupId='TRUE' /><Value Type='Lookup'>${id}</Value></Eq></Where>`);
    }
