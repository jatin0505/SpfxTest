...
export abstract class AsyncDataBase<T> implements IAsyncData<T> {
    ...
    public registerComponentForUpdates(component: IComponent) {
        ...
        component.attachUnmountHandler(this._componentIsUnmounting);
    }

    public unregisterComponentForUpdates(component: IComponent) {
        this._registeredComponents.delete(component);
        component.detachUnmountHandler(this._componentIsUnmounting);
    }

    private readonly _componentIsUnmounting = (component: IComponent) => {
        this._registeredComponents.delete(component);
        component.detachUnmountHandler(this._componentIsUnmounting);
    }
...
export class AggregatedAsyncData<T> extends AsyncDataBase<T[]> {
    private readonly _dataAsync: AsyncData<T | T[]>[];

    public readonly promise: Promise<T[]>;

    constructor(dataAsync: AsyncData<T | T[]>[]) {
        super();

        this._dataAsync = [...dataAsync];

        this.promise = Promise.all(this._dataAsync.map(d => d.promise)).then(a => _.flatten(a));
    }
    ...
    public get data(): T[] {
        return _.flatten(this._dataAsync.map(async => async.data instanceof Array ? async.data : [async.data]));
    }
